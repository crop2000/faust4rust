...
pub fn run_benchmark_arr(c: &mut criterion::BenchmarkGroup<WallTime>) {
    ...
    let input_array: &[&[f32]; 1] = ib.split_at(1).0.try_into().expect("too few input buffers");
    let mut output_array: &mut [&mut [f32]; 1] = ob
        .split_at_mut(1)
        .0
        .try_into()
        .expect("too few output buffers");

    // c.bench_function("flag_Apf_arr", |b| {
    c.bench_with_input("flag_Apf_arr", &mut output_array, |b, output_array| {
