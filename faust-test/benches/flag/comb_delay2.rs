...
impl mydsp {
        ...
        OutType: borrow::BorrowMut<[FaustFloat]>,
    {
        ...
        for output0 in zipped_iterators {
            self.fRec1[0] =
                self.fConst1 + (self.fRec1[1] - F64::floor(self.fConst1 + self.fRec1[1]));
            self.fVec0[(self.IOTA0 & 127) as usize] = unsafe {
                ftbl0mydspSIG0[(std::cmp::max(
                    0,
                    std::cmp::min((65536.0 * self.fRec1[0]) as i32, 65535),
                )) as usize]
            };
            self.fRec2[0] =
                self.fConst2 + (self.fRec2[1] - F64::floor(self.fConst2 + self.fRec2[1]));
                    ...
                    (F64::max(
                        ...
                        F64::min(
                            ...
                            5.0 * (unsafe {
                                ftbl0mydspSIG0[(std::cmp::max(
                                    0,
                                    std::cmp::min((65536.0 * self.fRec2[0]) as i32, 65535),
                                )) as usize]
                            } + 1.0),
                        ),
                    )) as i32,
...
pub fn run_benchmark(c: &mut BenchmarkGroup<WallTime>) {
    ...
    c.bench_function("flag_comp_delay2_old", |b| {
        b.iter(|| dsp.compute(black_box(buffer_size), black_box(ib), black_box(&mut *ob)))
    });
}
