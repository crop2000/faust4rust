...
Compilation options: -a ./bench_template.rs -lang rust -ct 1 -cn Apf -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 4
------------------------------------------------------------ */
mod flagvecvs4 {
    #![allow(clippy::all)]
    #![allow(unused_parens)]
    ...
    impl Apf {
        ...
        pub fn metadata(&self, m: &mut dyn Meta) {
            m.declare("compile_options", r"-a ./bench_template.rs -lang rust -ct 1 -cn Apf -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 4");
        ...
        pub fn compute_arrays(
            ...
            for (input0, output0) in zipped_iterators {
                ...
                for i in 0..output0.len() as i32 {
                    fZec0[i as usize] = fSlow1
                        * fRec0_tmp[(i32::wrapping_add(4, i32::wrapping_sub(i, 1))) as usize];
                    fRec0_tmp[(i32::wrapping_add(4, i)) as usize] = input0[i as usize]
                        - fSlow4
                            * (fSlow3
                                * fRec0_tmp
                                    [(i32::wrapping_add(4, i32::wrapping_sub(i, 2))) as usize]
                                - fZec0[i as usize]);
                }
                /* Post code */
                for j1 in 0..4 {
                    self.fRec0_perm[j1 as usize] =
                        fRec0_tmp[(i32::wrapping_add(vsize, j1)) as usize];
                }
                /* Vectorizable loop 1 */
                /* Compute code */
                for i in 0..output0.len() as i32 {
                    output0[i as usize] = fRec0_tmp
                        [(i32::wrapping_add(4, i32::wrapping_sub(i, 2))) as usize]
                        + fSlow4
                            * (fSlow3 * fRec0_tmp[(i32::wrapping_add(4, i)) as usize]
                                - fZec0[i as usize]);
                }
            }
        }
...
pub fn run_benchmark(c: &mut BenchmarkGroup<WallTime>) {
    ...
    let num_inputs = flagvecvs4::FAUST_INPUTS;
    let num_outputs = flagvecvs4::FAUST_INPUTS;
    ...
    let ibb = in_buffer
        ...
        .collect::<Vec<&[flagvecvs4::FaustFloat]>>();
    ...
    let mut obb = out_buffer
        ...
        .collect::<Vec<&mut [flagvecvs4::FaustFloat]>>();
    let ob = obb.as_mut_slice();

    c.bench_function("flagvecvs4_Apf", |b| {
...
pub fn run_benchmark_arr(c: &mut criterion::BenchmarkGroup<WallTime>) {
    ...
    c.bench_function("flagvecvs4_Apf_arr", |b| {
        b.iter(|| {
            let output_array = ob
                .split_at_mut(1)
                .0
