...
Compilation options: -a ./bench_template.rs -lang rust -ct 1 -cn Apf -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64
------------------------------------------------------------ */
mod flagvecvs64 {
#![allow(clippy::all)]
#![allow(unused_parens)]
...
impl Apf {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		m.declare("compile_options", r"-a ./bench_template.rs -lang rust -ct 1 -cn Apf -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64");
	...
	pub fn compute_arrays(&mut self, count: usize, inputs: &[&[FaustFloat] ; 1], outputs: &mut [&mut [FaustFloat] ; 1]) {
		...
		let mut fRec0_tmp: [F32;68] = [0.0;68];
		/* Main loop */
...
pub fn run_benchmark(c: &mut BenchmarkGroup<WallTime>) {
    ...
    let num_inputs = flagvecvs64::FAUST_INPUTS;
    let num_outputs = flagvecvs64::FAUST_INPUTS;
    ...
    .collect::<Vec<&[flagvecvs64::FaustFloat]>>();
    ...
    let ib = 
        ...
        .collect::<Vec<&mut [flagvecvs64::FaustFloat]>>();
        let ob = obb 
            .as_mut_slice();
    

    c.bench_function("flagvecvs64_Apf", |b| {
...
pub fn run_benchmark_arr(c: &mut BenchmarkGroup<WallTime>) {
    ...
    c.bench_function("flagvecvs64_Apf_arr", |b| {
        b.iter(|| {
            let output_array = ob
                .split_at_mut(1)
                .0
